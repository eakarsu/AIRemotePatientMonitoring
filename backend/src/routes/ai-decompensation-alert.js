// AI Predictive decompensation alerts
// Predict HF/COPD exacerbations 48-72h before symptoms
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const response = await fetch(baseUrl + '/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIRemotePatientMonitoring'
    },
    body: JSON.stringify({
      model: MODEL,
      messages: [
...
async function persistResult(userId, endpoint, inputData, result) {
  ...
  try {
    await pool.query(
      `CREATE TABLE IF NOT EXISTS ai_results (id SERIAL PRIMARY KEY, user_id INTEGER, endpoint VARCHAR(120), input_data JSONB, result TEXT, created_at TIMESTAMP DEFAULT NOW())`
    );
...
router.post('/', async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert AI assistant for AIRemotePatientMonitoring. Focus area: Predictive decompensation alerts. ${`Predict HF/COPD exacerbations 48-72h before symptoms`}. Respond ONLY with valid JSON (no markdown fences).`;
    const userPrompt = `Task: Predictive decompensation alerts.\n${`Predict HF/COPD exacerbations 48-72h before symptoms`}\n\nInput payload (JSON):\n${JSON.stringify(context, null, 2)}\n\nReturn JSON with shape:\n{ "summary": "...", "findings": ["..."], "recommendations": ["..."], "score": 0, "confidence": 0 }`;
    ...
    await persistResult(req.user?.id, 'decompensation-alert', context, parsed);
    res.json({ feature: 'decompensation-alert', model: MODEL, result: parsed });
  } catch (err) {
    console.error('[decompensation-alert]', err.message);
...
router.get('/history', async (req, res) => {
  try {
    const r = await pool.query('SELECT id, endpoint, input_data, result, created_at FROM ai_results WHERE endpoint=$1 ORDER BY created_at DESC LIMIT 50', ['decompensation-alert']);
